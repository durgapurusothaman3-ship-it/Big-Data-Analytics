...
------------------------
Pretends to be 3 "high-velocity" Flipkart warehouse servers
(Flipkart-Bangalore, Flipkart-Delhi, Flipkart-Hyderabad). Each one is
a tiny TCP server that, the moment a client (our harvester daemon)
connects, starts firing log lines continuously, forever, at random
intervals.
...
# One (name, port) per simulated warehouse server
BRANCHES = [
...
MESSAGE_TEMPLATES = {
    "INFO": [
        ...
        "Order#{oid} shipped via courier partner",
        "Order#{oid} out for delivery",
        "Order#{oid} delivered to customer",
    ],
    "WARNING": [
        "Order#{oid} delivery delayed by 1 day",
        "Warehouse stock running low for Order#{oid}",
        "High order volume detected near Order#{oid}",
        "Order#{oid} address verification pending",
    ],
    "ERROR": [
        "Payment gateway timeout for Order#{oid}",
        "Order#{oid} cancelled due to stock unavailability",
        "Courier partner API failure for Order#{oid}",
        "Order#{oid} failed pincode serviceability check",
    ],
    "DEBUG": [
        "Cache miss while fetching product details for Order#{oid}",
...
if __name__ == "__main__":
    ...
    print("\nAll simulated warehouse servers are up. Press Ctrl+C to stop.\n")
    try:
        while True:
            time.sleep(1)
